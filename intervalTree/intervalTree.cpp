...
Node* IntervalTree::createNode(int n,int max, Node* father){
    Node* nNode = new Node();
    nNode->interval.first = n;
    nNode->interval.second = max;
    nNode->key = max;
    nNode->left = NULL;
    nNode->right = NULL;
    nNode->father = father;
...
void IntervalTree::printNode(Node *node){
    if(node != NULL){
      std::cout<<'('<< node->interval.first <<","<<node->interval.second<<')'<< '\t';
...
void IntervalTree::insertWithFather(int n,int max,Node *father, Node **node){
    if(*node == NULL){
      *node = createNode(n,max,father);
      updateNode(*node,true);
    } else if(n < (*node)->interval.first)insertWithFather(n,max,*node,&(*node)->left);
    else insertWithFather(n,max,*node,&(*node)->right);
}
...
void IntervalTree:: right_right_Solve(Node* x, Node* y, Node* z){
  ...
  std::cout << "x,y,z: "<<x->interval.first<<" "<<y->interval.first<<" "<<z->interval.first << '\n';
  // step1: father child setup
  y->father = z->father;
  if(z->father != NULL){
    if(z->interval.first < z->father->interval.second) z->father->left = y;
  ...
  if(y -> left != NULL){
    ...
    if(y->left->key > z->key){
      z->key = y->left->key;
    } else z->key = z->interval.second;

    y->left->father = z;
  } else {
    // si y no tiene right, entonces z->key = z->highInterval
    z->key = z->interval.second;
  }
  // step 3:
  y->left = z;
  z->father = y;
...
void IntervalTree:: left_right_Solve(Node* x, Node* y, Node* z){
  // PARTE 1
    ...
    y->key = comp_keys(y->interval.second,var);
    // step 3:
    x->left = y;
    y->father = x;
  //  x->height = y->height;
...
void IntervalTree::deleteInterval(Node* node, int lo,int hi){
  if(node == NULL)std::cout << "NOT FOUND EXCEPTION" << '\n';
  else if(node->interval.first == lo && node->interval.second == hi){
    std::cout << "JUST FOUND" << '\n';
    printNode(node);
    if(isExternal(node)){
      Node* nr = node->right;
  ...
  } else if(node->interval.first > lo) deleteInterval(node->left,lo,hi);
  else deleteInterval(node->right,lo,hi);
}
...
bool IntervalTree::search(int lo, int hi, Node* currentNode){
  if(currentNode != NULL){
      if(lo <= currentNode->interval.second && currentNode->interval.first >= hi){
        std::cout << "FOUND: " << '\n';
        printNode(currentNode);
        return true;
      } else if(currentNode->left == NULL){
          search(lo,hi,currentNode->right);
      } else if(currentNode->left->key <= lo) {
          search(lo,hi,currentNode->right);
      } else {
          search(lo,hi,currentNode->left);
      }
  }
  return false;
}
...
void IntervalTree::insert(int n, int max){
  insertWithFather(n,max,NULL,root);
}
void IntervalTree::deleteInterval(int from,int to){
  deleteInterval(*root,from,to);
}
void IntervalTree::printIntevalTree(){
  std::cout << "NODE"<<'\t'<<"key"<<'\t'<<"height"<<'\t'<<"node"<<'\t'<<"Dir Father" << '\n';
  printIntevalTree(*root,"");
}
bool IntervalTree::search(int lo, int hi){
  return search(lo,hi,*root);
}
